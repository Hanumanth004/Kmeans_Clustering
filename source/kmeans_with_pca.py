...
#X_tmp = temp[:,0:k]
X_tmp = temp[:,0:2]
X_dim_reduced =np.dot(X_normalized, X_tmp)

new_means=[]
means=np.random.random((K,X_dim_reduced.shape[1]))
new_means=np.zeros_like(means)
num_assigned=np.zeros((K,1))

for k in xrange(max_iter):
    for i in xrange(X.shape[0]):
        X_tmp = X_dim_reduced[i]
...
for i in xrange(K):
    for k in range(X.shape[0]):
        if (cluster_num[k]==i):
            X_cluster.append(X_dim_reduced[k])
...
P,D,Q=np.linalg.svd(X_normalized)
X_pca = np.dot(X_normalized,Q[:,0:2])

fig = plt.figure(figsize=(8,8))
plt.scatter(X_pca[:,0],X_pca[:,1],c=cluster_num)
plt.title("K-means with PCA and only first principal components, K=20, max_iter=20")
plt.show()
